...
A tree that does not **compile** is a different matter from one that does not restore, and it
...
| A workspace whose design-time build fails | `projectInitializationComplete` fired and *every* probed project empty is the signature: the solution loaded and compiled nothing. That is not the same thing as source that does not compile — ordinary compile errors leave the workspace loaded and every query working, and are `diag`'s subject; this row is about the build that produces the *references*, and it fails whole projects at a time. Only in that state, and only after the wait has already failed, `cslq` runs `dotnet --version` in the root (exit 155 when a `global.json` pins an SDK nobody has) and looks for `obj/project.assets.json` per project, and names what it finds. A staged root went from 181.7 s with no cause to 23 s with one. |
