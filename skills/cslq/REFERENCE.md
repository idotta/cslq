...
5. **Check the symbol with `cslq def`** before concluding anything about `refs`.
   **A compile error is not the explanation.** Measured 2026-09-13 over seven break shapes and
   ...
   answering correctly, and every query at the break exit 1 rather than stale. The two windows
   worth knowing are both about a second wide and both self-correct: a file you have just
   written is not visible yet, and a file you have just deleted can still be named by a hit —
   whose header then says `(no longer on disk)`.
